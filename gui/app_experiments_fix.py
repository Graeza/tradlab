...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.ex_force_fixed_lot = QtWidgets.QCheckBox("Force fixed lot for Boom 1000/300 (0.2 / 0.5)")
    ...
    def apply_execution_guard(self):
        try:
            ...
            self.lbl_exec_guard_status.setText(
                f"spread={spread} (max {self.executor.max_spread_points}pt), "
                f"session={session} ({self.executor.session_start_hour}-{self.executor.session_end_hour}), "
                f"weekends={'ON' if self.executor.allow_weekends else 'OFF'}, "
